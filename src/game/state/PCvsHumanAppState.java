...
/**
 * Class describes behavior of the game in situation with one player
 *
 * @author Ivan
 */
public class PCvsHumanAppState extends AbstractAppState {
    private PCvsHumanAppState thisState;
    ...
    private final static Trigger TRIGGER_CLICK_ON_BUTTON = new MouseButtonTrigger(MouseInput.BUTTON_LEFT);
    private final static String MAPPING_CLICK_ON_BUTTON = "ClickOnButton";
    private ActionListener clickOnButtonActionListener = new ActionListener() {
        @Override
        public void onAction(String name, boolean isPressed, float tpf) {
            if (name.equals(MAPPING_CLICK_ON_BUTTON)) {
                ...
                if (results.size() > 0) {
                    ...
                    if (targetsName.equals("ResetButton") && !isPressed) {
                        ...
                        PCvsHumanAppState state = new PCvsHumanAppState();
                ...
                if (currentPlayer.doMove(inputManager, assetManager, cam, cubeNode, thisState)) {
                    try {
                        if (game.checkForWinner()) {
                            ...
                            changePlayer();
                            // bot's move
                            currentPlayer.doMove(inputManager, assetManager, cam, cubeNode, thisState);
                            if (game.checkForWinner()) {
                                showWinner(currentPlayer.getGameClass());
                                showWinnersConsequence();
                                stopGame();
...
//                cam.lookAt(new Vector3f(0, 0, 0), Vector3f.UNIT_Y);
    ...
    public void initialize(AppStateManager stateManager, Application app) {
        ...
        thisState = this;
        //this.settings = this.app.se

        cubeNode = new Node();
        rootNode.attachChild(cubeNode);
        game = new Game(makeScene(), LENGTH_FOR_WIN);

        currentPlayer = game.getCrossPlayer();
        rootNode.attachChild(SkyFactory.createSky(assetManager, "Textures/background5.jpg", true));
...
//        inputManager.addMapping(MAPPING_CLICK_ON_BUTTON, TRIGGER_CLICK_ON_BUTTON);
//        inputManager.addListener(clickOnButtonActionListener, MAPPING_CLICK_ON_BUTTON);
